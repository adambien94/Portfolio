...
export const clinicPlatformProject: ProjectDetail = {
  ...
  headline:
    "A production SSR web platform for a multi-location dental clinic group — one codebase serving three clinic brands and four languages, with all content driven by a headless CMS.",
  intro: [
    "I was the sole owner of the client application for this production platform. A single SSR application serves **three clinic brands** and **four languages**, with page content fetched from **Directus** via typed **GraphQL** at request time — fully decoupled from content management, so editors publish through the CMS and the site renders without redeploying copy changes.",
  ...
  highlight:
    "Four locales each have fully localized path segments — including Cyrillic slugs — and switching language navigates to the equivalent page, not the homepage.",
  sections: [
    {
      paragraphs: [
    ...
    {
      paragraphs: [
        "All content — offers, news, team members, price lists, custom pages — is fetched from Directus via colocated **`.gql` query files** that generate typed imports consumed at compile time, giving end-to-end safety on query variables and response shapes. Pages compose multiple queries where needed; paginated news refetches on page param change without full navigation.",
  ...
  closing: [
    "Built as the sole front-end owner: **SSR** rendering with async data caching, **Directus GraphQL** for typed content, localized URL routing across four languages, programmatic **SEO** (sitemap, hreflang, canonicals, JSON-LD), per-clinic theming, and REST + GraphQL orchestration for complex form submissions — production-ready with bot protection, analytics, and live chat integration.",
  ],
