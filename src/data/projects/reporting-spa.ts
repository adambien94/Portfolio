...
export const reportingSpaProject: ProjectDetail = {
  ...
  intro: [
    "This is a reporting application scoped by company ID in the URL. Multiple report pages share the same table, filter, and chart building blocks — wired to typed API methods through reusable composables rather than duplicated page logic.",
    "Cross-cutting concerns — authentication, API configuration, error handling — live in composable modules. The TypeScript client is **auto-generated from the backend OpenAPI spec** on every dev start and build, giving end-to-end type safety for DTOs, enums, and endpoints.",
  ],
  highlight:
  ...
  sections: [
    ...
    {
      paragraphs: [
        "Rather than building a separate table per report, a single **schema-driven table component** renders any `TableDto` from the API. Column metadata drives cell formatting — Polish decimal notation, dates, truncated strings — and a sticky summary row.",
    ...
    {
      paragraphs: [
        "A **`useVersionCheck`** composable embeds a build timestamp at compile time and polls `/version.json` periodically. When a new deployment is detected, users get a persistent notification to reload — ensuring long-lived sessions pick up fresh builds without stale cached bundles.",
      ],
    },
  ],
  closing: [
    "The approach is **composable-first** and **API-driven**: auth, column parsing, and version checking in reusable modules; global components shared across report pages; tables and charts that render from generic DTOs. Adding a new report means wiring existing building blocks — not rebuilding filters, tables, or export logic from scratch.",
  ],
